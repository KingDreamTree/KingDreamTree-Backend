...
class Settings(BaseSettings):
    ...
    #: 커버리지 컷 — 레퍼런스에서 보이는 팔다리 세그먼트 중 사용자 사진에서도
    #  보이는 비율이 이 미만이면 반려 (2026-08-16, 프론트 실측 리포트로 추가).
    #  전신 레퍼런스 대비 상반신만 잡힌 사진이 1차를 고득점 통과했다 — 안 보이는
    #  세그먼트는 비교에서 "제외"만 되고, 남은 각도끼리 비슷하면 점수가 높게 나온다.
    #  "레퍼런스에 나온 부위는 사용자 사진에도 있어야 한다"는 조건이 1차에 없었다
    #  (2차 프롬프트에는 의도만 있고 규칙 6·7이 이 케이스를 못 잡는다).
    #  방향은 비대칭이다 — 사용자가 레퍼런스를 덮어야 하고, 반대는 요구하지 않는다
    #  (상체 레퍼런스 + 전신 사용자 = 통과). 판정은 visibility 만 본다. 단축
    #  (foreshortening) 세그먼트는 각도만 못 잴 뿐 부위는 사진에 있으므로 커버로 친다.
    #  0.7: 무릎 아래 잘림(8개 중 6개, 0.75)은 통과, 골반 잘림(4개, 0.5)은 반려.
    #  ⚠️ 잠정값 — TOL·framing 전례처럼 실사진으로 튜닝할 것.
    pose_min_ref_coverage: float = 0.7
    ...
    #: 병합 없이도 **원래 이미 유효했던** 부위에 한해, 흡수된 옷 픽셀이 이
    #  비율을 넘으면 진단 비교 대상에서 뺀다 (2026-08-19, part_merge.py의
    #  "clothing_ratio 게이트" 예고를 구현).
    #  ⚠️ 병합 자체(오버레이 색칠)는 그대로 둔다 — 이 게이트는 is_valid만 건드린다.
    #  ⚠️ **처음부터 부족해서 병합으로 살아난 부위(긴팔 소매 등)는 대상이 아니다.**
    #     그런 부위는 병합이 곧 존재 이유이고, 병합 전에도 유효했던 부위만 이
    #     게이트를 통과시킨다 — 실측(2026-08-19): 반바지로 가려진 허벅지가
    #     원래 살만으로 이미 최소 기준의 20배를 넘겼는데도, 흡수된 옷이
    #     85~87%를 차지한 채 "유효한 비교 대상"으로 남아 있었다. 굳이 병합이
    #     필요 없던 부위가 병합으로 부풀려진 경우만 걸러낸다.
    seg_clothing_gate_ratio: float = 0.5

    # ------------------------------------------------------------------ #
    # 레이트리밋 — 인증 없는 엔드포인트 (#115)
    # ------------------------------------------------------------------ #
    #: POST /users — 인증 없는 유일한 쓰기 엔드포인트. 막지 않으면 스크립트로
    #  반복 호출해 뒤에 붙은 동기 Vision 호출(사진 업로드)까지 요금이 그대로
    ...
    vlm_provider: str = ""
    openai_api_key: str = ""
    # ⚠️ 다른 공급자 키는 두지 않는다. vlm.py 가 openai 전용이라(_call_json)
    #    쓰이지 않는 키가 남아 있으면 "값만 바꾸면 되겠지"라는 오해를 만든다.
    #    공급자를 늘리려면 vlm.py 에 분기를 먼저 구현해야 한다.
