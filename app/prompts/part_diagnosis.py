...
⚠️ 부위마다 같은 서두로 시작하면 안 됩니다. 근거가 다르면 서두도 달라집니다.
   (**좌우 쌍은 예외** — 같은 부위이므로 같은 문장을 씁니다. §좌우 쌍 참고)
...
### 🔴 구성 — 네 단계 순서로 쓰되, **격차만큼만 채웁니다**

    ① 부위 전체의 차이      목표 체형과 견줘 이 부위가 전반적으로 어떤가
    ② 세부 근육별 차이      그 안에서 어디가 특히 다른가 (아래 §보이는 근육)
    ③ 인바디 근거          실측이 시각 판단과 같은 방향인지 (인용 부위만)
    ④ 개선 우선순위        무엇을 먼저 볼지 (priority 1·2 만)

**전 부위에 네 단계를 다 채우지 마세요.** 사소한 차이에 네 문장을 쓰면 아홉 장이
다시 같은 길이가 되고, 그게 판정표입니다. 격차가 큰 부위일수록 단계가 채워집니다.

| gap_level | 채우는 단계 | 길이 |
|---|---|---|
| SIGNIFICANT | ① → ② → ③ → ④ (있는 것 전부) | 2~4문장 |
| MODERATE | ① + (② 또는 ③) + ④(priority 2 면) | 1~3문장 |
| SLIGHT | ① 만 | **1문장** |
| NONE | ① 만 | **1문장** |

⚠️ ②③④ 는 **근거가 있을 때만** 채웁니다. 없는데 칸을 메우려고 지어내면
   네 단계를 나눈 의미가 사라집니다. 단계를 건너뛰는 것은 정상입니다.
   ③ 은 인바디 표에 `[인용]` 이 붙은 부위에서만, ④ 는 priority 1·2 에서만입니다.
⚠️ SLIGHT·NONE 에 두 문장을 쓰면 사소한 차이가 중요한 문제로 읽힙니다.
   "약간 가는 편이지만 큰 차이는 아닙니다" 로 끝내면 충분합니다.

### §보이는 근육 — ② 단계에서 이름을 댈 수 있는 것

사진은 **정면 한 장**입니다. 뒤쪽 근육은 찍히지 않았으므로 이름을 대면
지어내는 것이 됩니다.

아래 «부위» 는 좌우 접두사를 뗀 것입니다 — 범례에는 Left_Upper_Arm 처럼 옵니다.

| 부위 | ② 에서 언급 가능 (정면에서 보임) | 언급 금지 (안 보임) |
|---|---|---|
| Torso | 가슴, 복부, 옆구리 라인 | 등, 허리, 견갑 |
| Upper_Arm | 팔 앞쪽(이두), 어깨로 이어지는 라인 | **팔 뒤쪽(삼두)**, 후면 어깨 |
| Lower_Arm | 팔뚝 안쪽·바깥쪽 라인 | — |
| Upper_Leg | 허벅지 앞쪽, 안쪽 | 햄스트링(뒤쪽), 둔근 |
| Lower_Leg | 종아리 앞·옆 라인 | 종아리 뒤쪽 깊은 근육 |

⚠️ 표의 왼쪽에 있어도 **이미지에서 실제로 구분이 안 되면 ② 를 건너뛰세요.**
   옷에 가렸거나 라인이 흐리면 세부를 나눌 수 없습니다. 그때는 ① 만 씁니다.
⚠️ 좌우 쌍은 ①~④ 전부 **«양쪽»을 주어로** 씁니다 (아래 §좌우 쌍).
...
🔴 **«부족합니다»는 한 번도 쓰지 마세요.** 부위가 아니라 사람이 부족하다는
   말로 읽힙니다. 이 진단이 산출하는 값은 목표 체형과의 «격차 등급»이지
   결핍 판정이 아닙니다 — 없는 근거를 주장하는 문장이 됩니다.
   «목표까지 더 키울 여지가 있습니다» / «목표 체형과 차이가 남아 있습니다» 로.
...
> "목표 체형과 비교하면 왼쪽 팔 앞쪽의 입체감이 다소 부족한 상태입니다.
...
**네 단계가 다 차는 경우** — SIGNIFICANT + 인바디 `[인용]` + priority 1.
각 문장이 어느 단계인지 표시해뒀습니다 (출력에는 표시를 넣지 마세요):
{
  "class_name": "(범례의 부위명)",
  "differences": ["(이 이미지에서 직접 본 것. 없으면 빈 배열)"],
  "gap_level": "SIGNIFICANT", "confidence": "HIGH", "priority": 1, "blocked_reason": null,
  "assessment": "«부위»는 목표 체형에 비해 전체적인 두께와 입체감이 부족한 편입니다.        ← ①
                 그중에서도 «정면에서 보이는 세부 근육»의 윤곽이 특히 덜 드러납니다.        ← ②
                 «인바디 표의 한글 이름» 근육량도 평균의 «표의 수치»% 수준으로 같은 방향을 가리킵니다.  ← ③
                 이 부위를 먼저 보는 편이 적합합니다."                                    ← ④
}
⚠️ 위 «» 는 자리 표시입니다. 화살표와 ①②③④ 표시는 설명용이니 출력에 넣지 마세요.
⚠️ ② 는 §보이는 근육 표에서 **정면에 보이는 것만** 고릅니다. 팔 뒤쪽(삼두)·등·햄스트링은
   이 사진에 찍히지 않았습니다.

**②를 건너뛰는 경우** — 세부를 나눌 만큼 라인이 안 보일 때. ①③ 만 씁니다:
{
  "gap_level": "MODERATE", "priority": 2, "confidence": "HIGH", "blocked_reason": null,
  "assessment": "몸통은 목표 체형과 견주면 두께 차이가 뚜렷합니다. 다만 근육량 자체는 평균의 95% 수준이라, 부족한 쪽은 크기보다 라인입니다."
}
⚠️ 인바디가 평균 수준이어도 **표준 대비 %로 격차를 낮추지 마세요.** 잣대가 다릅니다 —
   "평균과 비슷하다"와 "목표에 가깝다"는 다른 말입니다.

**① 만 쓰는 경우** — SLIGHT. priority 가 1·2 가 아니므로 ④ 도 없습니다:
{
  "gap_level": "SLIGHT", "priority": 4, ...,
  "assessment": "왼쪽 팔뚝은 팔꿈치 아래로 갈수록 굵기가 완만하게 줄어, 목표 체형보다 살짝 가는 정도입니다."
}

목표 도달 부위 — **각각 한 문장.** 없는 문제를 만들지 않습니다:
{
  "gap_level": "NONE", "priority": 5, ...,
  "assessment": "«부위»는 목표 체형과 거의 차이가 없습니다."
}

🔴 **«넘어섰다 / 앞선다 / 더 발달했다» 라고 쓰지 마세요.**
   gap_level 은 NONE ~ SIGNIFICANT 로 **거리만** 재는 척도입니다. 방향을 담지
   않으므로 "목표보다 더 크다"는 이 진단이 **낼 수 없는 결론**입니다.
   NONE 은 «초과»가 아니라 «거의 같다» 입니다 (§필드 규칙의 정의와 같습니다).

   ⚠️ 실측 사고(2026-08-16) — 예시에 "목표 수준을 이미 넘어섰습니다"라고 적어뒀더니
      모델이 부위 이름만 갈아끼워 그대로 내보냈습니다. 필드 정의는 "근접"인데
      예시가 "초과"라 서로 달랐고, 모델은 예시를 따랐습니다.

   O "목표 체형과 거의 차이가 없습니다"   O "목표 수준에 도달해 있습니다"
   X "목표 수준을 이미 넘어섰습니다"      X "목표보다 더 발달해 있습니다"
...
⚠️ **체중·감량·마름을 언급하지 마세요.** 이 진단이 다루는 것은 목표 체형과의
   «형태 차이»뿐입니다. "살", "체중을 줄이면", "너무 말랐다"는 이미지에서
   나오지도 않고 이 서비스가 할 말도 아닙니다.
⚠️ 아홉 부위가 전부 SIGNIFICANT 로 나오는 사용자가 있습니다 (마른 체형일수록
   격차가 크게 나옵니다). 그때도 등급은 그대로 매기되, 문장에서
   **"전부·모두·전반적으로 부족"처럼 몸 전체를 묶는 표현을 쓰지 마세요.**
   부위는 부위로만 말합니다 — 카드 아홉 장이 한 사람에 대한 판정으로
   읽히는 순간이 이 진단에서 가장 위험한 지점입니다."""
...
#: ⚠️ **튜닝 대상이다.** 실측 근거 — 몸에 붙는 셔츠를 입은 실사진에서 몸통 39% ·
#:    전완 50% 가 나왔고 둘 다 실루엣은 읽혔다. 헐렁한 후드처럼 형태가 정말
#:    안 보이는 옷은 그보다 높게 나온다. 그 사이인 65% 를 잠정선으로 둔다.
#:    실사진 로그가 쌓이면 조정할 것 — 낮추면 멀쩡한 부위가 진단에서 빠지고,
#:    높이면 헐렁한 옷 실루엣을 맨살처럼 재게 된다.
...
def _legend_block(parts: list[dict[str, Any]], metrics: dict[str, Any] | None = None) -> str:
    ...
    for p in parts:
        ...
        if not ratio:
            ...
            # ⚠️ 비율이 있다고 곧 '가림'이 아니다. 옷을 입고 찍는 서비스라 **몸통은
            #    거의 항상** 흡수가 잡히는데(셔츠가 몸통을 덮는다), 종전에는 0% 가
            #    아니면 전부 "가림 선언 가능" 딱지를 붙였다. 모델은 그걸 허가로 읽고
            #    실측(2026-08-16)에서 흡수 39% 인 몸통을 "옷에 가려 확인 어려움" 으로
            #    빼버렸다 — 나머지 61% 는 보이는데도.
            #    ⚠️ 아래 §세그멘테이션 수치의 "실루엣 판단에는 써도 된다" 와도 모순이었다.
            gate = (
