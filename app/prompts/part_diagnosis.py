...
### ① 차이가 있을 때 (SIGNIFICANT · MODERATE)
    ...
    «관찰 근거» 때문에 그렇게 보이는 것으로, 이번 비교에서 눈에 띄는 차이입니다.

  예) 목표 체형과 비교하면 상완은 볼륨이 다소 부족해 보입니다. 팔의 외곽선과
      전체적인 형태가 상대적으로 밋밋해서 그렇게 보이는 것으로, 이번 비교에서
      눈에 띄는 차이입니다.
...
### ② 차이가 작을 때 (SLIGHT · NONE)
  ...
  예) 목표 체형과 비교했을 때 전체적인 형태가 유사하게 보입니다. 지금은
      몸통보다 다른 부위의 차이가 더 크게 보여서, 여기는 아직 눈여겨볼
      단계는 아닙니다.
...
### ③ 판단 불가일 때 (gap_level 이 null)
  ...
  예) 사진만으로는 해당 부위의 외곽선이 충분히 보이지 않아서 목표 체형과의
      형태 차이를 정확히 비교하기는 어렵습니다.
