...
왼쪽/오른쪽이 같은 부위(왼쪽 위팔 ↔ 오른쪽 위팔)는 **gap_level · priority 를
...
⚠️ **다만 문장(assessment)의 주어는 «양쪽»이 아니라 «그 카드의 부위»입니다**
   (2026-08-20 개정). 사용자는 «왼쪽 위팔» 카드를 눌러서 그 카드만 보고 있는데
   "양쪽 위팔은…" 이라고 하면 지금 뭘 보고 있는지가 흐려집니다.
   **좌우를 묶어 부르는 것은 종합 진단의 몫입니다** — 거기서 한 번만 «양쪽»
   으로 합칩니다.

    O "왼쪽 위팔은 목표 체형보다 근육 윤곽이 덜 드러나는 편입니다."
    X "양쪽 위팔 모두 …"   ← 이 문장이 갈 곳은 종합 진단입니다

🔴 **한쪽 카드에만 해당하는 내용을 반대쪽에 쓰지 마세요.** 특히 인바디 수치는
   그 방향의 값입니다 — «왼팔 …%» 를 오른쪽 위팔 카드에 쓰면 명백한 오답입니다
...
def _legend_block(parts: list[dict[str, Any]], metrics: dict[str, Any] | None = None) -> str:
    ...
    for p in parts:
        ...
        #    (실측: 오른쪽 위팔 카드가 "왼팔 …%" 를 인용). 좌우를 묶어 부르는
