...
그렇습니다. 가려진 얼굴은 진단·관찰·사진 품질 판단 어디에도 쓰지 마세요.
...
### 몸통(Torso) — **옆선의 곡선 방향**을 보세요

몸통은 다른 부위와 달리 «굵기»보다 **외곽선이 어느 쪽으로 휘는가**가 형태를
결정합니다. 옷 위로도 읽히는 신호라 이 카드에서는 이걸 우선 보세요:

    · 허리 옆선이 **안쪽으로 들어가는가**(잘록함) **바깥으로 볼록한가**
    · 어깨에서 허리로 내려오며 폭이 좁아지는가, 거의 일자인가, 오히려 넓어지는가
    · 배 앞쪽이 앞으로 나와 옆에서 볼 때 곡선이 생기는가
    · 목표 체형의 같은 선과 견줘 **어느 방향으로** 다른가

⚠️ 이 관찰은 **두 사진의 옆선을 견주는 것**입니다. 한 장만 보고 "체형이 어떻다"
   고 규정하지 마세요.

## 곡선과 체성분을 연결할 때 — 조건이 있습니다

바깥으로 볼록한 옆선은 **체지방이 실루엣에 더 드러난 상태로 보일 수 있습니다.**
사용자가 실제로 알고 싶어 하는 연결이므로 말해도 됩니다. 단:

  🔴 **사진만으로 체지방률을 숫자로 말하지 마세요.** 사진은 체성분을 재지 못합니다.
     인바디가 주어졌다면 그 **실측 체지방률을 인용**하고 옆선 관찰과 연결하세요.
     인바디가 없으면 «보이는 형태»까지만 쓰고 체성분은 단정하지 마세요.

  🔴 **평가·비하가 되지 않게 쓰세요.** 읽는 사람이 본인입니다. "살쪘습니다",
     "체형이 나쁩니다" 류는 금지입니다. 관찰 어투(§어투)로, 목표와의 **차이**만
     짚습니다.

⚠️ 반대 방향도 같은 규칙입니다 — 옆선이 안으로 많이 들어가 있으면 그것도
   관찰로 쓰되, 마르다·부족하다 같은 가치 판단을 붙이지 마세요.
...
이미 목표에 도달한 부위입니다. "조금 더 늘리면 좋습니다"처럼 개선을 제안하거나
"유지하세요"처럼 방향을 붙이지 마세요 — 그것도 처방입니다. 차이가 없다는
관찰 한 문장으로 끝내세요.

⚠️ **«넘어섰다 / 앞선다» 라고도 쓰지 마세요.** gap_level 은 거리만 재는 척도라
...
| gap_level | 담을 것 |
|---|---|
| SIGNIFICANT · MODERATE | 아래 네 가지 중 **근거가 있는 것 전부**. 근거를 빼지 마세요 |
| SLIGHT · NONE | 전체 차이만 한 마디. 사소한 차이를 부풀리지 마세요 |
...
⚠️ 길이는 **근거가 하나 더 있을 때 느는 것**이지 등급이 높다고 늘리는 게
   아닙니다. 카드가 나란히 놓이므로 길이가 심하게 들쭉날쭉하면 읽는 리듬이
   무너집니다 (실측 2026-08-17) — 다만 **글자 수를 세지는 마세요.** 종전에는
   «40자/90자» 상한이 있었는데, 아홉 장을 그 상자에 넣으면 근거를 넣을 자리가
   없어 전부 «더 가늘어 보입니다» 한 줄로 수렴했습니다 (2026-09-10 폐기).
   근거를 다 쓰고 **군더더기를 빼는 방향**으로 짧게 만드세요.

## 🔴 세 가지 상황 — 요구하는 내용이 다릅니다

부위 카드는 **본 것을 말하는 곳**입니다. "그래서 무엇을 하라"는 종합 진단이 맡습니다.

🔴 **고정된 문장 틀을 쓰지 마세요.** 아래는 상황별로 **담을 내용**이지 베낄
   문장이 아닙니다. 실측(2026-08-19) — "«부위»는 «무엇»이 «어떻게» 보입니다"
   골격을 주자 다섯 부위 전부 어미까지 똑같은 문장으로 나왔습니다. 골격을
   남겨두면 «» 자리만 바뀌고 나머지는 그대로 복사됩니다. 문장은 매번 처음부터
   새로 짜세요 — 카드가 나란히 놓이므로 **모양이 비슷한 두 문장은 그 자체로
   결함입니다.**

### ① 차이가 있을 때 (SIGNIFICANT · MODERATE)

담을 내용: **무엇이 다른가** (사진에서 직접 본 것 — 두께·굵기·윤곽선·근육의
경계·형태의 굴곡 중 실제로 보인 것) + **무엇을 보고 그렇게 판단했는가**
(관찰 근거). 두 가지가 같은 말의 반복이면 안 됩니다.
...
마세요 — 이 틀의 목적은 "여기는 지금 걱정할 곳이 아니다"를 알려주는 것입니다.
...
🔴 **반드시 이 두 표현을 그대로 쓰세요**: "충분히 보이지 않아서", "비교하기는
   어렵습니다". ①②와 섞이면 사용자는 판단이 내려진 줄 압니다.
   가려진 원인(옷·각도·잘림)은 **이 사진에서 실제로 그런 것만** 쓰세요 —
   가려지지 않은 부위에 남의 사유를 붙이지 마세요.
   **여기에 어떤 방향·처방도 붙이지 마세요.** 못 본 부위입니다.
...
🔴 **재지 못하는 것을 재지 마세요** (종합 진단과 같은 기준입니다):
...
⚠️ 인바디 수치가 주어진 부위는 그 값을 **인용**할 수 있습니다. 인용은 측정이
   아닙니다 — 실측값을 옮기는 것이고, '평균 대비'임을 밝히면 됩니다.
...
부위 어디에나 쓸 수 있는 말이라 카드가 전부 똑같아집니다.

쓸 것은 **그 부위의 선과 면이 실제로 어떻게 생겼는가** 입니다. 선이 어디서
부풀고 어디로 좁아지고 어떤 기울기로 이어지는지를 보고, **목표 사진의 같은
지점**은 그것이 어떻게 다른지를 쓰세요.
...
🔴 **«부족합니다» 로 문장을 끝내지 마세요.** 사람이 부족하다고 읽힙니다.
   이 진단이 내는 값은 레퍼런스와의 «격차»이지 결핍 판정이 아닙니다.

   X "왼쪽 상완이 부족합니다."                    ← 사람에 대한 판정으로 읽힘
   O "상완의 볼륨이 다소 부족하게 보입니다."       ← 관찰 대상은 «볼륨»이고 관찰 어투
   O "목표까지 더 키울 여지가 있습니다."
...
같은 값으로** 보내세요. 실측 좌우 차이는 대개 3% 안쪽이라 등급을 가를 근거가
못 됩니다 — 좌우가 실제로 크게 다를 때만 등급을 다르게 매깁니다.

⚠️ **다만 문장(assessment)의 주어는 «양쪽»이 아니라 «그 카드의 부위»입니다**
   (2026-08-20 개정). 사용자는 «왼쪽 상완» 카드를 눌러서 그 카드만 보고 있는데
   "양쪽 상완은…" 이라고 하면 지금 뭘 보고 있는지가 흐려집니다.
   **좌우를 묶어 부르는 것은 종합 진단의 몫입니다** — 거기서 한 번만 «양쪽»
   으로 합칩니다.

    O "왼쪽 상완은 목표 체형보다 근육 윤곽이 덜 드러나는 편입니다."
    X "양쪽 상완 모두 …"   ← 이 문장이 갈 곳은 종합 진단입니다

🔴 **한쪽 카드에만 해당하는 내용을 반대쪽에 쓰지 마세요.** 특히 인바디 수치는
   그 방향의 값입니다 — «왼팔 …%» 를 오른쪽 상완 카드에 쓰면 명백한 오답입니다
   (실측 2026-08-20: 오른쪽 카드가 왼팔 수치를 인용했습니다). 그 카드의 부위에
   해당하는 값만 쓰고, 없으면 수치를 빼세요.

⚠️ 두 카드의 **내용이 결과적으로 비슷해지는 것은 괜찮습니다** — 실제로 비슷하니까요.
   억지로 다르게 만들려고 없는 좌우 차이를 지어내지는 마세요.
...
- **같은 팔다리의 이웃 부위** (허벅지↔종아리, 상완↔전완): 처방을 부위의
  ...
  전완은 쥐는 힘. 일반 상식 수준의 역할만 쓰고 지어내지 마세요.
- 내용이 같아도 **어미·연결어·문장 구조를 부위마다 다르게** 쓰세요. 카드
  9장을 나란히 놓았을 때 두 장 이상이 같은 틀로 읽히면 그 자체가 결함입니다.
...
⚠️ **완성 예문은 여기 두지 않습니다.** 종전에는 이 자리에 본보기 카드가 한 장
   있었는데, 두 가지가 동시에 잘못이었습니다 — ① 예문이 있으면 모델이 관찰보다
   예문 재현을 택합니다(2026-08-19 실측: 골격을 주자 다섯 부위가 어미까지 똑같이
   나왔습니다). ② 그 예문의 주어가 «양쪽 상완» 이라, 바로 위 §좌우 쌍이 금지한
   문장을 본보기로 제시하고 있었습니다. 조건만 지키고 문장은 매번 새로 쓰세요.
...
### 🔴 부위 카드에는 운동 방향을 쓰지 않습니다

**본 것만 쓰고 끝내세요.** 위 세 문형 어디에도 "무엇을 하세요"는 없습니다.
...
🔴 종목 이름은 물론이고 **동작 계열·방향도 쓰지 않습니다.**

   X "덤벨 컬과 같은 팔 운동을 통해 강화하세요"      ← 종목
   X "팔을 굽혀 드는 계열에 볼륨을 싣는 방향입니다"  ← 계열도 마찬가지
   O "팔의 외곽선이 상대적으로 밋밋해서 그렇게 보이는 것으로, 상완의 발달 차이가 눈에 띕니다"

   방향은 **종합 진단(silhouette)** 이 전체를 보고 한 번만 말합니다. 부위마다
   방향을 달면 카드 9장이 전부 "~하세요"로 끝나고, 그 방향들이 서로 맞는지는
   아무도 보증하지 않습니다.
...
⚠️ **아래는 가상의 다른 사용자입니다.** 지금 진단하는 사진·수치와 무관합니다.
   말투만 따르고 **내용을 복사하지 마세요.** 특히 "옷에 가렸다" 같은 관찰은
   예시 사용자의 것입니다 — 지금 이미지에서 직접 확인한 경우에만 쓰세요.
...
def _pair_subject(name_ko: str) -> str:
    """'왼팔 전완' → '전완' — «양쪽» 뒤에 붙일 좌우 없는 이름."""
    for prefix in ("왼팔 ", "오른팔 ", "왼쪽 ", "오른쪽 "):
        if name_ko.startswith(prefix):
            return name_ko[len(prefix) :].strip() or name_ko
    return name_ko
...
def _inbody_block(
    ...
    lines.append(
        ...
        '   X "왼쪽 전완 근육량이 평균의 82% 수준"  ← 전완만 잰 값이 아닙니다\n'
