...
def _metrics_block(metrics: dict[str, Any], parts: list[dict[str, Any]]) -> str:
    ...
    **정밀해 보이는 노이즈**고, VLM 에게 "이 수치와 모순되지 말라"고
    ...
    out = [
        "※ 크기 비교 수치는 제공하지 않습니다 — 두 사진은 각도·거리·자세가 달라 성립하지 않습니다.\n"
...
def _symmetry_block(
    ...
    lines.append(
        ...
        "픽셀과 인바디 실측이 어긋나면 좌우 근육량은 실측이 정답입니다 — 실측 차이가\n"
...
def _inbody_block(
    ...
    lines.append(
        ...
        "※ 표준 대비 %는 일반인 평균과의 비교입니다 — 목표 체형과의 격차(gap_level) 근거로\n"
        "  쓰지 마세요. 못 봐서 인바디로 판단한 부위만 예외입니다."
    )
    return "\n".join(lines)
