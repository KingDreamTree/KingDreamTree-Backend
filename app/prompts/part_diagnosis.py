...
1. **인바디 실측** — 세그먼트 단위, 제출됐을 때만. 체내 실측이라 옷·조명에 안 흔들림
2. **같은 사진 안의 좌우 대칭** — 한 장 안에서 잰 것이라 각도·거리가 소거됨
3. **이미지 관찰** — 두 사진의 실루엣·비례를 눈으로 비교

⚠️ **두 사진 사이의 크기 수치는 주어지지 않습니다.** 촬영 각도·거리·자세가 달라
   픽셀 크기 비교가 성립하지 않기 때문입니다. 크기 차이는 3번(이미지 관찰)으로만
