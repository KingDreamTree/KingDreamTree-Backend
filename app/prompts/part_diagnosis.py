...
🔴 **문장 길이 예산** — **문장 하나는 60자 안팎, assessment 전체 200자 이내.**
   ⚠️ 2026-08-17에는 이보다 훨씬 빡빡했습니다(전체 90자) — 그때는 부위 카드
      9장이 한 화면에 동시에 나란히 놓여 높이를 맞춰야 했습니다. 지금 화면은
      부위를 하나씩 골라 그 아래 패널 하나에만 보여주고, 그 패널은 높이가
      내용에 맞춰 늘어나는 구조라 카드 간 높이를 맞출 이유가 사라졌습니다
      (2026-08-20 재확인). §길이 표의 문장 수만큼은 실제로 다 쓰세요 —
      등급이 높다고 길게 쓰는 게 아니라 **근거가 하나 더 있을 때 문장이 하나
      느는** 것이니, 짧게 자르려고 근거를 버리지 마세요.
...
🔴 **재지 못하는 것을 재지 마세요** (종합 진단과 같은 기준입니다):
    ...
    · 실제 근육량 · 체지방률
    · **근력** — 사진도 인바디도 근력은 재지 않습니다. "근력 차이" 판정 금지
  ...
  ⚠️ **완성 예문은 두지 않습니다** — 하나만 주면 그 문장 틀이 부위와 무관하게
     반복됩니다. 대신 **한 사진 안에서의 상대적 비율**이라는 조건만 지키고,
     이 부위에서 실제로 두드러진 관계로 매번 새로 쓰세요. 관계의 종류는
     다양합니다 — 예를 들어 허리 대비 어깨 폭, 몸통 대비 팔 두께, 위팔 대비
     팔뚝 굵기처럼 **이 카드가 다루는 부위와 그 이웃 사이의 비율**이면 됩니다.
     위 예시들도 문장으로 베끼지 말고 «어떤 종류의 관계를 보는가»만 참고하세요.
...
🔴 **구체적인 운동 종목 이름을 쓰지 마세요.** 인과 주장이 없어도 안 됩니다.
   ...
   약속한 것과 사용자가 실제로 받는 것이 어긋납니다. **동작 계열·방향도 같은
   이유로 쓰지 않습니다** — 아래 «부위 카드에는 운동 방향을 쓰지 않습니다» 참고.
...
🔴 **«부족합니다» 로 문장을 끝내지 마세요.** 사람이 부족하다고 읽힙니다.
   ...
   ⚠️ **완성 예문은 두지 않습니다.** 담을 것: 사람이 아니라 «관찰 대상»(볼륨·
      윤곽·라인 등, 이 부위에서 실제로 본 것)을 주어로, «~한 편입니다» 류의
      완곡한 어투로 매번 새로 쓰세요.

🔴 **이식 테스트 — 한 카드의 문장을 다른 부위 카드에 옮겨 붙여도 말이 되면,
   그 문장은 관찰이 아니라 상투구입니다.** 그런 문장은 버리고 이 부위에서만
   나올 수 있는 내용으로 다시 쓰세요. 어느 부위에나 붙는 문장이 여러 카드에
   반복되는 것이 이 진단의 가장 흔한 실패 형태입니다.
...
⚠️ 문장은 반드시 **«양쪽»을 주어로** 쓰고, 한쪽만 가리키는 말("왼팔이", "오른쪽은")을
   ...
   틀린 말이 됩니다.
   (예문은 일부러 두지 않습니다 — 예전 이 자리의 예문이 실사용 출력에서 격차
   크기와 무관하게 거의 모든 카드에 변형 반복되는 만능 문장이 됐습니다.)
...
⚠️ 따라 쓸 예문은 일부러 두지 않습니다 — 예전 이 자리의 예문("~입체감이 덜
   드러나는 편입니다" 류)이 격차가 크든 작든 거의 모든 카드에 변형 반복되는
   만능 문장이 됐습니다. 아래 조건만 지키고 문장은 이 부위에서 실제로 본
   것으로 매번 새로 짜세요.

- 관찰은 **구체적인 시각 표현**으로 — 입체감, 윤곽, 라인, 실루엣, 두께, 경계
  중에서 **이 부위에서 실제로 본 것** (§이식 테스트)
...
🔴 종목 이름은 물론이고 **동작 계열·방향도 쓰지 않습니다.**
   ...
   ⚠️ **완성 예문은 두지 않습니다.** 종목·계열 없이 «무엇이 보이는가»만 —
      이 부위에서 실제로 관찰한 윤곽·라인·두께로 이 사용자의 문장을 새로
      쓰세요.
    ...
    for class_name, segment in part_to_segment.items():
        ...
        if parts_desc:
            ...
            if citation_targets.get(segment) == class_name:
                # ⚠️ "수치를 씁니다"만 지시하면 숫자 한 줄이 문장에 그대로 삽입된다
                #    ("근육량이 평균의 89.9% 수준입니다"). 수치는 이미지 관찰과
                #    결합해 해석할 때만 정보가 된다.
                mark = (
                    " **[인용]** ← 이 부위에서만 수치를 씁니다."
                    " 숫자만 덧붙이지 말고 이미지에서 본 형태와 연결해 해석하세요"
                )
