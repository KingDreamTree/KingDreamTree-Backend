...
⚠️ 인바디는 좌우 팔·다리와 몸통 5단위라 **위팔과 팔뚝을 구분하지 못합니다.**
...
🔴 **문장 길이 예산** — **문장 하나는 40자 안팎, assessment 전체 90자 이내.**
   ...
   섞이면 카드마다 높이가 달라져 읽는 리듬이 무너집니다 (실측 2026-08-17).
...
🔴 **«부족합니다» 로 문장을 끝내지 마세요.** 사람이 부족하다고 읽힙니다.
   ...
   X "왼쪽 위팔이 부족합니다."                    ← 사람에 대한 판정으로 읽힘
   O "위팔의 볼륨이 다소 부족하게 보입니다."       ← 관찰 대상은 «볼륨»이고 관찰 어투
...
왼쪽/오른쪽이 같은 부위(왼쪽 위팔 ↔ 오른쪽 위팔, 왼쪽 허벅지 ↔ 오른쪽 허벅지)는
...
⚠️ 문장은 반드시 **«양쪽»을 주어로** 쓰고, 한쪽만 가리키는 말("왼팔이", "오른쪽은")을
   ...
   틀린 말이 됩니다.
    ...
    O "양쪽 위팔 모두 목표 체형보다 근육 윤곽이 덜 드러나는 편입니다."
    X "왼쪽 위팔이 목표 체형보다 가늡니다."   ← 오른쪽 카드에도 이 문장이 갑니다

🔴 **«왼쪽 …은 / 오른쪽 …은» 으로 문장을 시작하면 위반입니다.** 좌우 쌍 카드의
   주어는 언제나 «양쪽 …» 입니다. 실측(2026-08-19): 이 규칙을 두고도
   "왼쪽 팔뚝은 …" / "오른쪽 팔뚝은 …" 으로 나뉘어, 뒤 문장이 완전히 같은
   두 카드가 만들어졌습니다 — 좌우를 나눈 의미도, 다르게 쓴 의미도 없습니다.
   좌우 쌍은 **한 번 쓰고 두 카드에 같은 문장을 넣는다**고 생각하세요.
...
> "양쪽 위팔 모두 목표 체형과 비교하면 팔 앞쪽의 입체감이 덜 드러나는 편입니다.
>  팔의 전체적인 라인과 두께가 상대적으로 밋밋해서 그렇게 보이는 것으로, 이번 비교에서 눈에 띄는 차이입니다."
...
🔴 종목 이름은 물론이고 **동작 계열·방향도 쓰지 않습니다.**
   ...
   O "팔의 외곽선이 상대적으로 밋밋해서 그렇게 보이는 것으로, 위팔의 발달 차이가 눈에 띕니다"
...
def _legend_block(parts: list[dict[str, Any]], metrics: dict[str, Any] | None = None) -> str:
    ...
    for p in parts:
        ...
        # ⚠️ 좌우 쌍도 **부위 줄에** 박는다. "«양쪽»을 주어로 쓰라"는 전역 규칙이
        #    세 번 무시됐다 (실측 2026-08-19 — "왼팔 전완은 …" / "오른팔 전완은 …"
        #    으로 나뉘고 뒷문장은 완전히 같은 두 카드가 나왔다). 모델이 이 항목을
        #    생성할 때 다시 읽는 곳은 이 줄이므로 짝과 주어를 여기서 지정한다.
        pair = _pair_of(name, {q["class_name"] for q in parts})
        if pair:
            gate += (
                f"\n  🔴 **좌우 쌍: {pair} 과(와) 한 쌍입니다.** 두 카드에 **똑같은 문장**을"
                f" 넣고, 주어는 «양쪽 {_pair_subject(p.get('name_ko') or name)}» 으로 쓰세요"
                f" — «왼/오른»으로 시작하면 위반입니다."
            )
