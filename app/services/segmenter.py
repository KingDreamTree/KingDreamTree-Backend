...
def segment(
    ...
    #    ⚠️ 2026-08-19에 한 번 껐다가 다시 켰다 — DB body_part_segment의
    #    invalid_reason CHECK 제약이 이 값을 몰라 매 세그멘테이션이
    #    23514(제약 위반)로 죽는 프로덕션 장애가 있었다(refit.live 업로드 2건
    #    실패). db/migrations/2026-08-19_mostly_clothing_reason.sql 로 제약을
    #    갱신한 뒤에만 다시 켠다 — 이 순서를 지키지 않으면 같은 장애가 재발한다.
    if settings.seg_merge_clothing:
