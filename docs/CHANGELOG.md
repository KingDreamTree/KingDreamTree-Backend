...
## 2026-09-12 - Day 드리프트 · 부위 카드 같은 멘트 · 프롬프트 DB 버전 관리

**무엇이**: 전문은 docs/refactoring-20260912.md
- Day 가 «완료 횟수 + 1» 이라 중복 완료 기록마다 밀렸다 → 마지막 완료 기록 기준 (FE #264)
    + 새로고침에도 남는 완료 잠금 · 중복 정리 스크립트 (#190, 46건 미삭제)
- 부위 카드 9장이 같은 문장 → 규칙 1,737자로 압축 · 두 경로 한 벌 · 문장 구조 지정 삭제 ·
    부위 카드만 gpt-4.1 (`vlm.PART_MODEL`) (#189 #191 #194). 문형반복 중앙값 8 → 1
- 프롬프트 13개를 `prompt_version` 테이블로 — SQL 로 버전 올리고 되돌림, 진단마다 버전 기록 (#192 #193)

⚠️ 백엔드 배포가 09-08 부터 EC2 SSH 실패 — main 이 운영에 안 올라가고 있다.
