...
### 갤러리 업로드의 거울 처리 — 2026-08-17 리허설에서 실제로 터졌습니다

리허설 실측 — **비대칭 레퍼런스(한 손 공·한 손 허리)에서 갤러리 업로드가 유사도
0%로 떨어졌습니다.** 원인은 방향입니다. 갤러리 경로는 정방향 판정인데,

- 사람은 사진을 **거울처럼** 따라 합니다 (화면 왼쪽에 보이는 손 = 내 왼손), 그리고
- 삼성 기기 전면 카메라는 설정에 따라 셀피를 **반전 저장**합니다.

둘 중 하나면 팔 세그먼트 각도가 좌우로 뒤집혀 `hard_tol_deg` 를 넘고
(`JOINT_TOO_FAR`) → **정확히 0%**. 다리처럼 수직인 부위는 반전해도 각도가
같아서 **대칭 자세에서는 이 문제가 안 보입니다.**

세 가지를 구현해주세요 (`web/pose-score.js` 최신본 필요 — `mirrorSuspected` 와
`MESSAGES.MIRROR_SUSPECTED` 가 추가됐습니다):

1. **체크박스** "거울로 찍은 사진(좌우 반전)" — 위 표대로 `UPLOAD` 경로에만, 기본 꺼짐
2. **체크했으면 로컬 판정도 거울 기준으로**:
   `evaluate(mirrorLandmarks(refLm), userLm, criteria, ...)` — 실시간 촬영과 같은
   호출입니다. 업로드에 `is_mirrored=true` 를 실으면 서버가 저장 직전에 사진과
   랜드마크를 되돌립니다. 규칙은 하나입니다 — **판정 방향 = 사진 방향.**
3. **힌트**: 정방향 판정이 자세 사유로 떨어졌을 때
   `mirrorSuspected(refLm, userLm, criteria, opts)` 가 `true` 면
   `MESSAGES.MIRROR_SUSPECTED` 를 띄우고 체크박스를 유도하세요.

⚠️ 힌트가 떴다고 **판정을 몰래 거울 기준으로 바꿔 통과시키면 안 됩니다.**
저장되는 사진의 좌우 방향이 사용자마다 뒤섞여 좌우 부위 진단이 무의미해집니다.
방향 전환은 사용자의 신고(체크박스) → `is_mirrored=true` 재업로드로만.

### 실시간 촬영(CAPTURE)의 저장 방향 — 2026-08-18 개정

**촬영본은 거울 방향으로 저장합니다.** 셔터 순간에 캔버스를 좌우 반전해서 굽고,
랜드마크도 `mirrorLandmarks()` 로 **세트로 반전**해서 함께 보냅니다. 이렇게 하면
저장본이 미리보기에서 본 그대로이자 레퍼런스와 같은 방향이 되어, 저장 이후의
모든 처리(세그·비교·진단·표시)가 **직접 업로드와 완전히 같은 기준**이 됩니다.
교차 짝짓기·표시 반전이 필요 없습니다. `is_mirrored` 는 여전히 `false` 로
보냅니다 — 프론트가 방향을 이미 확정했으므로 서버가 되돌릴 일이 없습니다.
