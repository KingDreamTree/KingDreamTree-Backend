...
- **홈 진입(온보딩 [시작하기])마다 저장된 `user_id` 를 지우고 `POST /users` 로 새로 받습니다.**
  세션이 도는 동안만 로컬에 유지하는 값입니다
- 앱 진입 시 `GET /users/me` 로 먼저 확인하세요. 서버 DB가 초기화됐는데 옛 id가 남아 있으면 이후 요청이 전부 실패합니다
- 새로고침으로 흐름을 되살리는 것은 그대로입니다 — 재발급은 **홈에서 다시 시작할 때만** 입니다

⚠️ **«최초 1회 발급 후 영구 보관» 은 폐기된 정책입니다** (#169, 2026-09-08).

옛 `user_id` 가 남아 있으면 `POST /sessions` 가 409 `ACTIVE_SESSION_EXISTS` 로 옛 세션을
돌려주고, 프론트가 그걸 이어받는다. 그 세션의 `workout_log` 가 살아 있어서 **처음 들어온
사용자에게 "Day 4" 게이지가 뜨고, 코치가 하지도 않은 운동을 "오늘 하신 운동"으로 다뤘다.**
같은 브라우저를 계속 쓰면 이 승계가 매번 누적된다.

**돌아왔을 때 기록이 없는 것이 의도된 동작이다.** 시연 중심으로 확정된 방침이고,
#166(익명 로그인)이 들어오면 그때 다시 본다.

⚠️ 홈 진입마다 `POST /users` 가 나가므로 `user_create_rate_limit`(기본 30/분/IP) 를 탄다.
행사장처럼 여러 명이 한 IP 를 쓰는 환경에서 429 가 나면 이 값을 올린다 (`config.py`).
EC2 는 Caddy 뒤라 `--proxy-headers` 가 없으면 IP 가 전부 caddy 로 잡혀 상한이 전체
공용이 된다 — `Dockerfile` 에 이미 들어가 있다.
...
## ~~거울 매칭 세션의 표시 규약 (cross_paired)~~ — 2026-08-18 폐지

`comparable.cross_paired` 필드는 하위 호환을 위해 남아 있지만 **이제 항상
`false`** 입니다. 실시간 촬영이 사진·랜드마크를 거울 방향으로 저장하도록 바뀌어
([7번](#7-거울-촬영-처리)), 교차 색칠·사용자 사진 표시 반전이 **전부 필요
없어졌습니다.** 관련 분기(`mirrorDisplay`, 교차 색칠, 안내 캡션)는 프론트에서
제거해도 됩니다. 단일 관문은 여전히 `app/services/part_pairing.py` — 되돌릴
일이 생기면 서버 쪽 그 함수 하나만 살리면 됩니다.
...
⚠️ **HEIC 는 서버만 디코드할 수 있습니다. 브라우저(크롬)는 못 합니다.** 랜드마크
추출을 프론트가 하므로, **파일 선택 `accept` 에서 `image/heic` 를 빼주세요**
(레퍼런스·사용자 사진 둘 다). 넣어두면 삼성 기기의 '고효율 사진(HEIF)'이 선택돼
디코드 단계에서 "분석하지 못했어요"로 떨어집니다 — 사용자에게는 유사도 0%처럼
보입니다.
...
규칙은 하나입니다 — **판정 방향 = 저장 방향 = 화면에 보이는 방향.**

| 경로 | 판정 | 저장 |
|---|---|---|
| 실시간 촬영 (`CAPTURE`) | 레퍼런스를 반전해 채점 (프리뷰가 거울이므로) | **사진·좌표를 세트로 반전**해 화면에서 본 방향으로 |
| 파일 업로드 (`UPLOAD`) | 사진 그대로 정방향 | 파일 그대로 |

두 경로 모두 저장본이 레퍼런스와 같은 방향이 되므로, 저장 이후 처리(세그·비교·
진단·표시)가 완전히 같습니다. 서버의 교차 짝짓기와 화면의 표시 반전은 그래서
필요 없어졌습니다 (2026-08-18 개정).

⚠️ `is_mirrored` 는 **양쪽 다 `false`** 로 보냅니다. 프론트가 방향을 이미 확정하므로
서버가 되돌릴 일이 없습니다. (서버는 이 필드를 계속 지원하지만 지금 쓰는 경로가
없습니다 — 되살릴 때만 씁니다.)

⚠️ 알려진 한계 — 갤러리에 **거울 셀카**를 올리면 좌우가 뒤집힌 채 저장되고,
비대칭 레퍼런스면 자세 점수가 0으로 떨어질 수 있습니다(반전된 팔이 `hard_tol_deg`
초과 → `JOINT_TOO_FAR`). 사용자 신고에 의존하던 체크박스 경로를 걷어낸 결과이며,
그런 사용자는 실시간 촬영으로 유도하는 것이 현재 방침입니다.

### 실시간 촬영(CAPTURE)의 저장 방향

셔터 순간에 캔버스를 좌우 반전해서 굽고, 랜드마크도 `mirrorLandmarks()` 로
**세트로 반전**해서 함께 보냅니다.

```js
// 프리뷰는 CSS 반전 그대로:  video, canvas.overlay { transform: scaleX(-1); }
// 셔터 — 사진과 좌표를 "세트로" 뒤집는다
ctx.translate(w, 0); ctx.scale(-1, 1); ctx.drawImage(video, 0, 0);
upload({ file, poseLandmarks: mirrorLandmarks(liveLm), ... });
```

⚠️ **위험은 "한쪽만 뒤집기"입니다.** 사진만 뒤집고 좌표는 원본으로 보내면(또는
반대) 좌우가 통째로 어긋나는데 **에러는 하나도 안 납니다** — 왼팔 진단이
오른팔에 붙습니다. 사진과 좌표는 반드시 같은 방향으로 움직여야 합니다.

⚠️ 이 개정 이전에 촬영된 세션은 원본 방향으로 저장돼 있어 새 기준과 어긋납니다.
과거 세션은 재촬영이 맞습니다.

⚠️ 알려진 트레이드오프 — 저장본의 "왼팔"은 실제 몸의 오른팔입니다. 인바디 좌우
실측을 진단에 섞는 경로와 어긋나므로 백엔드(B)와 처리 방침 협의가 필요합니다.
