...
> ✅ **2026-08-13 확정 — `HIGHLIGHT` 입니다. 크롭 파일은 만들지 않습니다.**
> 담당 B가 원본 사진 + 라벨 맵으로 **호출 시점에 메모리에서** 하이라이트를 만들어 VLM에 넣습니다. 따라서
> - `body_part_segment.crop_path` / `crop_bucket` 은 **계속 NULL**
> - `body-parts` 버킷은 **미사용** (생성은 돼 있으나 아무것도 안 들어감)
> - 담당 A는 라벨 맵 PNG 외에 **추가 산출물이 없습니다**
>
> 스키마는 그대로 둡니다. 컬럼을 지우면 나중에 크롭 방식으로 돌아갈 때 마이그레이션이 필요해지고, NULL 컬럼 두 개의 비용은 없습니다. **이게 크롭을 NULL 허용으로 강등해둔 설계가 실제로 값을 한 지점입니다.**

아래는 크롭 방식을 택했을 경우의 원래 설계입니다 (참고용).

- VLM 입력 형식이 "부위 크롭"으로 확정되면, 워커가 맵 + 원본에서 크롭을 잘라 `body-parts` 버킷에 저장하고 `crop_path`를 채웁니다.
...
| 10 | ~~레퍼런스 프리셋~~ | — | ✅ **확정 (2026-08-13): 도입 안 함.** `USER_UPLOAD` 고정 |
