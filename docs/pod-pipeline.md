...
- **대기열 초과 · 처리 스레드 없음**: 업로드가 503 `POD_BUSY` — 프론트는 "잠시 후 다시". 실측(2026-09-09, 로컬 RTX 5060, 대기열 3): 동시 5명 → 4명 접수(응답 4~6초), 1명 503, 4명 전부 완료까지 132초(1명당 약 33초), 끝나면 메모리 사진 0
- **부위 부족**: 세그 뒤 비교 가능한 부위가 2개 미만이면 `VLM_PART` 가 FAILED + "비교 가능한 부위가 N개뿐입니다 (최소 2개). 다시 촬영해주세요." 프론트는 이 문구로 재촬영을 안내한다 (종전엔 이 문구가 일반 오류로 바뀌어 나갔다 — `app/worker/run.py` 2026-09-09 수정). 정상 흐름에서는 스크리닝(팔·다리 잘림 CROPPED)이 먼저 걸러 드물다
...
- 가리는 방식 `POD_FACE_MASK_STYLE`: **blur**(기본, 얼굴 박스에 내접하는 **타원 안만** 강한 가우시안 = 짧은 변/6 — 이목구비가 사라지고 피부색·머리 윤곽만 남는다. 타원이라 목 옆·승모근은 손대지 않는다) | fill(회색 사각형). 둘 다 진단 차이는 아래 실측에서 흔들림 범위 안이었다. blur 가 기본인 이유는 사진처럼 보여 GPT 가 "가림"을 품질 문제로 볼 여지가 적기 때문
