...
**비용** — 시간당 과금입니다. 개발·시연 때만 켜고 끄면 실제 비용은 크지 않습니다. 끄기 전에 진행 중인 잡이 없는지 확인하세요 — `PROCESSING` 상태로 멈춘 잡은 재시도 대상이 되지 않습니다.

> ⚠️ **PROCESSING인 채로 팟을 끄면 그 잡은 영영 안 끝납니다.** `attempts`는 이미 올라갔고 `status`는 `PROCESSING`이라 `claim()`이 다시 집지 않습니다. 지금은 수동으로 `PENDING`으로 되돌려야 합니다.
> → 개선안: `started_at`이 N분 이상 지난 `PROCESSING` 잡을 `PENDING`으로 되돌리는 정리 작업. **아직 구현 안 됨.**
