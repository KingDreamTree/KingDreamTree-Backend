...
| # | 작업 | 완료 기준 |
|---|---|---|
| 1-1 | **Sapiens2 실제 클래스명·개수 확인** | 추론 1회 돌려 라벨 목록 출력 → **B에게 공유** |
| 1-2 | `seed_body_parts.py` 작성·실행 | 전체 클래스 + `is_comparable` 9개 + `color_hex` |
...
> **1-1이 다른 모든 일보다 먼저입니다.** `body_part` seed가 확정돼야 B의 프롬프트 부위 목록이 정해집니다.
