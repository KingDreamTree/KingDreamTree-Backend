...
- ⚠️ `body_part` seed는 A가 만듭니다. 부위 목록이 아직 없으면 임시로 9개를 넣고, **A가 실제 클래스명을 확인하면 갈아엎으세요.** 프롬프트에 부위명을 하드코딩하지 말고 DB에서 읽으세요.
...
**A에게서 받아야 할 것** — Sapiens2 실제 클래스 목록, 맵 PNG 샘플 1장 + `label_map` JSON. **이게 오기 전까지는 §5 Phase 1의 더미 데이터로 진행합니다.**
