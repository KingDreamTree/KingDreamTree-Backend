...
준비
    1) 점수별 사진을 만든다. 두 가지 방법:
       * web/pose-live.html — 자동 촬영된 사진의 파일명이 곧 점수다 (내려받기).
         단 통과(70+)한 순간만 찍히므로 낮은 점수 구간은,
       * 아무 카메라로 찍은 사진을 web/score-photos.html 에 넣어 점수를 확인하고
         파일명을 그 점수로 바꾼다.
       점수가 대략 100 / 90 / 80 / 70 / 60 이 되도록 한 장씩.
...
def main() -> int:
    ...
    print(f"     web/pose-live.html 로 몇 번 따라 서보면 본인이 몇 점대인지 바로 나옵니다.")
    return 0
