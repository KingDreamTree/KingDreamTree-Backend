...
def test_baseline_separation() -> None:
    ...
    for label, needle in [
        # ⚠️ 2854032 에서 보고서체 → 설명체로 바뀌었다 ("~로 확인됩니다" →
        #    "~때문에 그렇게 보이는 것으로"). 문형 자체는 셋 다 살아 있으므로
        #    **현재 문구**로 검사한다. 옛 문구를 계속 찾으면 테스트만 빨개진다.
        ("차이 있음 문형", "이번 비교에서 눈에 띄는 차이입니다"),
        ("차이 작음 문형", "다른 부위의 차이가 더 크게 보여서"),
        ("판단 불가 문형", "정확히 비교하기는 어렵습니다"),
    ]:
