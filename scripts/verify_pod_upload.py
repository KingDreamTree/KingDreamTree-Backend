...
def jpeg(w: int = 600, h: int = 800, color=(120, 100, 90)) -> bytes:
    """단색 + 얼굴 자리(랜드마크 y 0.2~0.4 근처)에 어두운 무늬 — 얼굴 가림이 바이트를 바꾸게."""
    from PIL import ImageDraw

    img = Image.new("RGB", (w, h), color)
    ImageDraw.Draw(img).rectangle(
        (int(w * 0.45), int(h * 0.2), int(w * 0.55), int(h * 0.35)), fill=(20, 20, 20)
    )
    buf = io.BytesIO()
...
def landmarks_json() -> str:
    return json.dumps(
        [
            {"index": i, "x": 0.5, "y": 0.2 + i * 0.02, "z": 0.0, "visibility": 0.95}
            for i in range(33)
        ]
    )
...
check(
    "기준 사진에 파일을 보내면 409 PHOTO_PIPELINE_POD",
    r.status_code == 409 and r.json()["error"]["code"] == "PHOTO_PIPELINE_POD",
    str(r.status_code),
)
...
check(
    "  storage_path 없음 · signed_url null",
    PHOTOS["REFERENCE"]["storage_path"] is None and r.json()["signed_url"] is None,
)
...
check(
    "사용자 사진 전이면 upload-token 409",
    r.status_code == 409 and "USER" in json.dumps(r.json()),
    str(r.status_code),
)
...
check(
    "사용자 사진 자세(거리) 미달 → 422 POSE_MISMATCH",
    r.status_code == 422 and r.json()["error"]["code"] == "POSE_MISMATCH",
    str(r.status_code),
)
...
check(
    "  storage_path 없음 · job_id null",
    PHOTOS["USER"]["storage_path"] is None and r.json()["job_id"] is None,
)

r = api.post(f"{BASE}/analysis", headers=H)
check(
    "POST /analysis → 409 PHOTO_PIPELINE_POD",
    r.status_code == 409 and r.json()["error"]["code"] == "PHOTO_PIPELINE_POD",
    str(r.status_code),
)
...
check(
    "  토큰이 세션·사용자를 담고 검증됨",
    payload["session_id"] == SESSION_ID and payload["user_id"] == USER_ID,
)
check(
    "  응답에 팟 주소가 없음",
    "pod" not in json.dumps(r.json()).lower() or "url" not in json.dumps(r.json()).lower(),
)

PHOTOS["USER"]["crop_box"] = {
    "x": 1,
    "y": 2,
    "w": 3,
    "h": 4,
...
check(
    "GET /photos/user → signed_url null, crop_box 그대로",
    r.status_code == 200 and r.json()["signed_url"] is None and r.json()["crop_box"]["w"] == 3,
    r.text[:200],
)
...
r = api.post(
    ...
    headers=H,
    json={"items": [{"bucket": "photos", "path": f"{USER_ID}/x.jpg"}]},
)
...
def files():
    return {
        "reference": ("r.jpg", jpeg(), "image/jpeg"),
        "user": ("u.jpg", jpeg(900, 700), "image/jpeg"),
    }
...
check(
    "토큰 없음 → 401",
    r.status_code == 401 and r.json()["error"]["code"] == "INVALID_UPLOAD_TOKEN",
    str(r.status_code),
)
...
r = pod.post(
    "/upload",
    files=files(),
    headers={"X-Upload-Token": fresh_token(), "Content-Length": str(10**9)},
)
...
async def screen_reject(user_image, reference_image):
    return photo_screening.ScreenResult(
        suitable=False, reason="LOOSE_CLOTHING", message="옷이 헐렁합니다", confidence="HIGH"
    )
...
check(
    "스크리닝 반려 → 422 UNSUITABLE_PHOTO",
    r.status_code == 422 and r.json()["error"]["code"] == "UNSUITABLE_PHOTO",
    r.text[:200],
)
check(
    "  반려 시 파이프라인에 넘기지 않음 (잡 0 · 메모리 사진 0)",
    not OPENED and photo_source.held() == 0,
)
...
r = pod.post("/upload", files=files(), headers={"X-Upload-Token": fresh_token()})
check(
    "스크리닝 불가 → 503 SCREENING_UNAVAILABLE",
    r.status_code == 503 and r.json()["error"]["code"] == "SCREENING_UNAVAILABLE",
    str(r.status_code),
)
...
check(
    "  잡 3개(SEG×2, VLM_PART)가 PROCESSING 으로, source=pod",
    [j["kind"] for j in OPENED] == ["SEG_REFERENCE", "SEG_USER", "VLM_PART"]
    and all(j["payload"].get("source") == "pod" for j in OPENED),
    str([j["kind"] for j in OPENED]),
)
check("  jobs 반환", body.get("jobs", {}).get("VLM_PART") == "j-3", str(body.get("jobs")))
cb = body.get("crop_box", {}).get("USER") or {}
check(
    "  crop_box: 900x700 가로 사진은 3:4 로 좌우가 잘림",
    cb.get("w") == 525 and cb.get("h") == 700 and cb.get("source_width") == 900,
    json.dumps(cb),
)
check(
    "  기준 사진(600x800, 이미 3:4)은 크롭 없음",
    (body.get("crop_box", {}).get("REFERENCE") or {}).get("w") == 600,
)
check(
    "  가공본 크기·crop_box 가 행에 기록됨",
    PHOTOS["USER"].get("width") == 525
    and PHOTOS["USER"].get("height") == 700
    and PHOTOS["USER"].get("crop_box", {}).get("w") == 525,
    str((PHOTOS["USER"].get("width"), PHOTOS["USER"].get("height"))),
)
check(
    "  두 장이 메모리에 등록됨 (held_photos 2)", photo_source.held() == 2, str(photo_source.held())
)
check(
    "  메모리의 가공본이 JPEG", (photo_source.get(SESSION_ID, "USER") or b"")[:3] == b"\xff\xd8\xff"
)
...
check(
    "  응답 face_masked 두 장 true",
    body.get("face_masked") == {"REFERENCE": True, "USER": True},
    str(body.get("face_masked")),
)
...
if vlm_copy and seg_copy:
    ...
    # 단색 사진(검증용)은 블러해도 같은 색이라, 박스 안 비교는 진짜 사진 조각으로 한다
    from PIL import ImageDraw as _D

    probe = Image.new("RGB", (200, 260), (120, 100, 90))
    _D.Draw(probe).rectangle((80, 30, 120, 70), fill=(20, 20, 20))  # 얼굴 자리에 어두운 무늬
    probe_lms = [
        {"index": i, "x": 0.5, "y": 0.2 + i * 0.02, "z": 0.0, "visibility": 0.95} for i in range(33)
    ]
    blurred, bbox = face_mask.apply(probe, probe_lms, "blur")
    filled, fbox = face_mask.apply(probe, probe_lms, "fill")
    check(
        "  blur: 박스 안 무늬가 뭉개짐 (원본과 다름)",
        bbox is not None and blurred.crop(bbox).tobytes() != probe.crop(bbox).tobytes(),
        str(bbox),
    )
    check(
        "  blur: 박스 밖은 그대로",
        bbox is not None and blurred.getpixel((5, 255)) == probe.getpixel((5, 255)),
    )
    check(
        "  fill: 박스 안이 회색",
        fbox is not None and filled.getpixel((100, 50)) == face_mask.FILL,
        str(filled.getpixel((100, 50))),
    )
    from app.config import settings as _settings

    check(
        "  기본 style 은 blur",
        _settings.pod_face_mask_style == "blur",
        _settings.pod_face_mask_style,
    )
...
check(
    "pipeline=quick → 202, 세그 잡 없이 VLM_PART(mode=quick)만",
    r.status_code == 202
    and OPENED[-1]["kind"] == "VLM_PART"
    and OPENED[-1]["payload"].get("mode") == "quick"
    and len(OPENED) == 4,
    str([j["kind"] for j in OPENED]),
)
