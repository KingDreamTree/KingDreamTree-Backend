...
def contract_prompt() -> None:
    ...
    for label, needle in (
        ...
        ("판정 대신 형태 묘사", "실제로 보인 선과 면을 구체적으로"),
        ("목표와 비교 필수", "목표 체형의 같은 자리와 견줍니다"),
        ("길이는 차이만큼", "길이는 차이만큼"),
        ("부위 이름으로 시작하지 않기", "부위 이름으로 문장을 시작하지 말고"),
        ("좌우 쌍 옮겨 쓰지 않기", "첫 카드를 옮겨 쓰지 않습니다"),
        ("옷·사진 조건은 문장 밖으로", "카드 문장에는 쓰지 않습니다"),
        ("옷이 아니라 몸", "옷이 아니라 몸을 비교"),
        ("정면이라 뒤쪽 금지", "정면 사진이라 뒤쪽"),
        ("운동 방향 금지", "부위 카드에는 운동 방향을 쓰지 않습니다"),
        ("인용 부위만 수치", "[인용] 표시가 붙은 부위"),
        ("어투 — 진단체", "진단체(~한 편입니다"),
        ("좌우: «양쪽»으로 묶지 않기", "«양쪽»으로 묶지 말고"),
    ...
    check("시작 틀 배정 없음 (2026-09-12 걷어냄)", "문장 방식:" not in prompt)
